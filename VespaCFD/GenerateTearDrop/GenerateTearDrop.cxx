...
// Start of a util which will generate parametric shapes - this is based on:
// http://www.vtk.org/Wiki/VTK/Examples/Cxx/PolyData/WeightedTransformFilter
// so far a banana but I hope to make it an aerodynamic shape.
// The idea is to take a sphere and stretch it in one direction only
...
int main(int argc, char *argv[])
{
  ...
  vtkSmartPointer<vtkFloatArray> tfarray = vtkSmartPointer<vtkFloatArray>::New();
...
//    vtkSmartPointer<vtkFieldData>::New();
  ...
  vtkSmartPointer<vtkTransform> rotated = vtkSmartPointer<vtkTransform>::New();
  ...
  vtkSmartPointer<vtkActor> weightedTransActor = vtkSmartPointer<vtkActor>::New();
